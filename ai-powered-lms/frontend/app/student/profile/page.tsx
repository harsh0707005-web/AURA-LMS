...
export default function StudentProfilePage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiRequest("/profile", {
        method: "PUT",
        body: JSON.stringify({ name, department, enrollmentNo }),
      });
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 min-h-screen">
      <Topbar title="Student Profile & Institutional Information" subtitle="Academic Record Details" />
      ...
      <main className="flex-1 p-6 max-w-3xl mx-auto w-full space-y-6 animate-fade-in">
        <div className="bg-white border border-slate-200 rounded-2xl p-7 shadow-xs text-xs space-y-6">
          <div className="flex items-center space-x-4 pb-5 border-b border-slate-100">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900">{user?.name || "Student"}</h2>
              <p className="text-slate-500 font-mono text-[11px] mt-0.5">{user?.email}</p>
              <span className="inline-block mt-2 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-800 text-[10px] font-bold border border-blue-200">
                ROLE: {user?.role}
              </span>
            </div>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block font-semibold text-slate-700 mb-1.5">Enrollment Number</label>
                <input
                  type="text"
                  value={enrollmentNo}
